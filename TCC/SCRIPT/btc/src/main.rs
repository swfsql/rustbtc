...
fn main() {
  ...
  let msg_tx = btc::Msg::Msg::new_from_hex(&msg_tx_hex).unwrap();
  println!("{:?}", msg_tx);
  ...
  let msg_ping = btc::Msg::Msg::new(msg_ping_it.by_ref()).unwrap();
  println!("{:?\n}", msg_ping);
  ...
  let msg_pong_hex = "".to_string() +
    ...
    "0194102111e2af4d";
  let msg_pong = btc::Msg::Msg::new_from_hex(&msg_pong_hex).unwrap();
  println!("{:?}", msg_pong);
...
//println!("======================================================================");
 ...
 //"9C7C00000100000000000000E615104D00000000010000000000000000000000000000000000FFFF0A000001208D010000000000000000000000000000000000FFFF0A000002208DDD9D202C3AB457130055810100";
  let msg_version = btc::Msg::Msg::new_from_hex(&msg_version_hex).unwrap();
  println!("{:?}", msg_version);


 let msg_verack_hex = "F9BEB4D976657261636B000000000000000000005DF6E0E2";
 let msg_verack = btc::Msg::Msg::new_from_hex(&msg_verack_hex).unwrap();
 println!("{:?}", msg_verack);
