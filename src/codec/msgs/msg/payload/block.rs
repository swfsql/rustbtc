...
impl NewFromHex for Block {
        ...
        I: IntoIterator<Item = &'a u8>,
    {
        ...
        let block_header =
            BlockHeaders::new(it.by_ref()).chain_err(cf!("Error at creating a new Block Header"))?;
        ...
        for i in 0..txn_count_usize {
            let aux =
                Tx::new(&mut it).chain_err(cf!("Error at creating a new Tx, at txns {:?}", i))?;
            txns.push(aux);
        }
...
impl IntoBytes for Block {
    fn into_bytes(&self) -> Result<Vec<u8>> {
        ...
        let mut txn_count_vec = self.txn_count.into_bytes().chain_err(cf!(
