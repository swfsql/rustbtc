    ...
    match value_head {
      ...
      0xFD => {
        ...
        let value_body = Cursor::new(&aux)
          ...
          .chain_err(|| format!("(Commons::var_uint) Failed when VarUint tried to read {:?} as u16", aux))?;
      ...
      0xFE => { // ler 32 bit
        ...
        let value_body = Cursor::new(&aux)
          ...
          .chain_err(|| format!("Commons::var_uint) Failed when VarUint tried to read {:?} as u32", aux))?;
      ...
      0xFF => { // ler 64 bit
        ...
        let value_body = Cursor::new(&aux)
          ...
          .chain_err(|| format!("Commons::var_uint) Failed when VarUint tried to read {:?} as u64", aux))?;
